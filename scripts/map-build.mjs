...
for (const kind of ["paintings", "objects", "buildings", "posters", "prints", "photographs", "poems", "songs", "quotes", "links", "people", "writing", "bookmarks"]) {
  ...
  files.forEach((file) => {
    ...
    items.push({
      ...
      ...(fm.collection ? { collection: fm.collection } : {}),
      ...(fm.collection_city ? { collectionCity: fm.collection_city } : {}),
      ...(fm.collection_url ? { collectionUrl: fm.collection_url } : {}),
      ...(fm.display_status ? { displayStatus: fm.display_status } : {}),
      ...(fm.status_checked ? { statusChecked: fm.status_checked } : {}),
      ...(Number(fm.height_cm) > 0 ? { heightCm: Number(fm.height_cm) } : {}),
      ...(Number(fm.width_cm) > 0 ? { widthCm: Number(fm.width_cm) } : {}),
